...
class Parser:
    ...
    def parse(self):
        ...
        while self.current_tok.type != TT_EOF:
            if self.current_tok.type == TT_KEYWORD and self.current_tok.value in ('ENDEF', 'ENDIF', 'ENDCLASS', 'ENDWHILE', 'ENDFOR'):
    ...
    def statement_list(self, end_keywords):
        ...
        while (
            self.current_tok.type != TT_EOF and
            not (self.current_tok.type == TT_KEYWORD and self.current_tok.value in end_keywords)
        ):
    ...
    def call(self):
        ...
        while True:
            if self.current_tok.type == TT_LPAREN:
                ...
                res.register_advancement()
                self.advance()
                atom = ListAccessNode(atom, index_node)
        ...
        if self.current_tok.type in (TT_PLUSPLUS, TT_MINUSMINUS):
            if not isinstance(atom, (VarAccessNode, GetAttrNode, ListAccessNode)):
                return res.failure(InvalidSyntaxError(
                    atom.pos_start, self.current_tok.pos_end,
        ...
        if self.current_tok.type == TT_EQ:
            if isinstance(atom, GetAttrNode):
                ...
                if res.error: return res
                return res.success(SetAttrNode(atom.object_node, atom.attr_name_tok, value))
    ...
    def fun_def(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'DEF'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'DEF'"))
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'ENDEF'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'ENDEF'"
            ))
    ...
    def class_def(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'INHERITS'):
            ...
            self.advance()
            if self.current_tok.type != TT_IDENTIFIER:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected superclass name"))
        ...
        while self.current_tok.type != TT_EOF and not self.current_tok.matches(TT_KEYWORD, 'ENDCLASS'):
            if not self.current_tok.matches(TT_KEYWORD, 'DEF'):
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'DEF' (methods) inside class body"))
            
            method_node = res.register(self.fun_def())
            if res.error: return res
            if not method_node.var_name_tok:
                return res.failure(InvalidSyntaxError(method_node.pos_start, method_node.pos_end, "Methods inside a class must have a name"))
            
            method_nodes.append(method_node)
        
        if not self.current_tok.matches(TT_KEYWORD, 'ENDCLASS'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'ENDCLASS'"))
    ...
    def new_instance(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'NEW'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'NEW'"))
        ...
        if self.current_tok.type != TT_LPAREN:
                     return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected '(' after class name for 'NEW'"))
