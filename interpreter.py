...
from values import Number, String, Function, Class, List
from nodes import *
...
class Interpreter:
    def visit(self, node, context):
        ...
        if isinstance(result, RTResult) and (result.should_return or result.should_break or result.should_continue or result.error):
            return result
    ...
    def visit_StatementListNode(self, node, context):
        ...
        for statement_node in node.statement_nodes:
            ...
            if res.error or res.should_return or res.should_break or res.should_continue:
                return res
    ...
    def visit_ClassNode(self, node, context):
        ...
        if node.superclass_node:
            ...
            if not isinstance(superclass, Class):
                return res.failure(RTError(
                    node.superclass_node.pos_start, node.superclass_node.pos_end,
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        if node.op_tok.type in (TT_PLUSPLUS, TT_MINUSMINUS):
            ...
            if isinstance(target_node, VarAccessNode):
                ...
                value = context.symbol_table.get(var_name)
                if not value:
                    return res.failure(RTError(target_node.pos_start, target_node.pos_end, f"'{var_name}' is not defined", context))
                ...
                return res.failure(RTError(target_node.pos_start, target_node.pos_end, "Invalid target for increment/decrement", context))

            if not isinstance(value, Number):
                return res.failure(RTError(target_node.pos_start, target_node.pos_end, "Operand must be a number", context))
        ...
        if node.op_tok.type == TT_MINUS:
            if isinstance(number, Number):
                ...
                error = RTError(node.pos_start, node.pos_end, "Unary '-' can only be applied to numbers", context)
